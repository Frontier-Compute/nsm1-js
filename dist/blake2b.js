...
export function hexToBytes(hex) {
  if (typeof hex !== "string" || hex.length === 0 || hex.length % 2 !== 0) {
    throw new TypeError("hex must be a non-empty even-length string");
  }
  if (!/^[0-9a-fA-F]+$/.test(hex)) {
    throw new TypeError("hex contains non-hexadecimal characters");
  }
...
export function computeLeafHash(eventType, walletHash, serialNumber) {
  ...
  if (typeof walletHash !== "string") {
    throw new TypeError("walletHash must be a string");
  }

  if (eventType === "PROGRAM_ENTRY") {
    const walletBytes = ENCODER.encode(walletHash);
    if (walletBytes.length > 65535) {
      throw new RangeError("walletHash exceeds 65535 UTF-8 bytes");
    }
  ...
  if (eventType === "OWNERSHIP_ATTEST") {
    if (typeof serialNumber !== "string") {
      throw new TypeError("serialNumber must be a string");
    }
    const walletBytes = ENCODER.encode(walletHash);
    const serialBytes = ENCODER.encode(serialNumber);
    if (walletBytes.length > 65535 || serialBytes.length > 65535) {
      throw new RangeError("ownership fields exceed 65535 UTF-8 bytes");
    }
...
export function nodeHash(left, right) {
  if (!(left instanceof Uint8Array) || left.length !== 32) {
    throw new TypeError("left child must be 32 bytes");
  }
  if (!(right instanceof Uint8Array) || right.length !== 32) {
    throw new TypeError("right child must be 32 bytes");
  }
...
export function commitRoot(leafCount, rawRoot) {
  if (!(rawRoot instanceof Uint8Array) || rawRoot.length !== 32) {
    throw new TypeError("raw root must be 32 bytes");
  }
  let count;
  if (typeof leafCount === "bigint") {
    count = leafCount;
  } else if (typeof leafCount === "number" && Number.isSafeInteger(leafCount)) {
    count = BigInt(leafCount);
  } else {
    throw new TypeError("leaf_count must be a safe integer number or bigint");
  }
  if (count <= 0n) throw new RangeError("leaf_count must be positive");
  if (count > 0xffffffffffffffffn) throw new RangeError("leaf_count exceeds u64");
...
export function walkProof(leafHashHex, proof, leafCount) {
  if (typeof leafHashHex !== "string" || !/^[0-9a-fA-F]{64}$/.test(leafHashHex)) {
    throw new TypeError("leaf hash must be exactly 32 bytes of hex");
  }
  if (!Array.isArray(proof)) throw new TypeError("proof must be an array");
  if (proof.length > 64) throw new RangeError("proof exceeds 64 steps");
  let current = hexToBytes(leafHashHex);
  const steps = [];

  for (let index = 0; index < proof.length; index++) {
    if (!(index in proof)) throw new TypeError("proof must not contain holes");
    const step = proof[index];
    if (!step || typeof step !== "object" || Array.isArray(step)) {
      throw new TypeError("proof step must be an object");
    }
    if (typeof step.hash !== "string" || !/^[0-9a-fA-F]{64}$/.test(step.hash)) {
      throw new TypeError("proof sibling must be exactly 32 bytes of hex");
    }
